...
class DbManager(val context: Context) {
    ...
    @SuppressLint("Range")
    suspend fun readDbData(searchTex:String) : ArrayList<ListItem> = withContext(Dispatchers.IO){
        val dataList = ArrayList<ListItem>()
        val selection = "${MyDbNameClass.COLUMN_NAME_TITLE} like ?"
            val cursor = db?.query(MyDbNameClass.TABLE_NAME, null, selection, arrayOf("%$searchTex%"), null, null, null)
